# Intelligent Automation Pipeline — B2B Lead Pipeline
...
- **Supabase** (Postgres) — system of record: `pipeline_runs`, `actions`, `outcomes`, `pipeline_errors`, `engagement_events`, `app_config`, `campaigns`/`cadence_steps`, plus the MMM-foundation objects `funnel_stages` / `campaign_objectives` / `mmm_account_action_mix` (2026-07-07) + `dashboard_*` / `console_*` views. Also hosts one **pg_cron** job — `objective-status-daily` (07:30 UTC, `update_objective_status()`) — the only scheduled job that lives in the DB rather than n8n.
...
## Funnel objectives & MMM foundation (added 2026-07-07)

On top of the flow above, every campaign now carries **one limited, funnel-staged objective** (a success metric, a numeric target, and a window — e.g. *reply-or-meeting, target 1, within 21 days* at the `cold` stage). Brief Approval Handler auto-creates it (`Create Campaign Objective` node → `campaign_objectives`), pulling the objective from the matched template. A daily pg_cron job (`update_objective_status()`) flips each objective **met**/**missed** within its window. Every account×campaign then produces one row in `mmm_account_action_mix` — the channel mix used plus whether the objective was hit. That view is the training data for eventual channel-effectiveness modeling.

**Important framing:** this is the measurement *foundation* — an account-level attribution/uplift dataset — **not** classical spend-based Marketing Mix Modeling. The regression on top of it is future work, gated on real traffic volume (and, for causal answers, on adding randomized holdouts to the cadence). Applied via `supabase/2026-07-06_mmm_instrumentation.sql`.
...
1. **Supabase**: run `supabase/schema.sql` against a fresh project, then apply the dated migrations in order (`2026-07-06_mmm_instrumentation.sql`, `2026-07-07_trim_gate1_legacy.sql`) — these hold the authoritative schema changes since the base file was last regenerated. Also enable `pg_cron` and re-create the `objective-status-daily` job.
